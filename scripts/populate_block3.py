...
# Parse abilities from Bulbapedia text
abilities_text = """
...
209 | Disguise | Once per battle, the shroud that covers the Pokémon can protect it from an attack at the cost of 12.5% of its HP. | VII
...
# Parse the abilities
abilities = []
for line in abilities_text.strip().split('\n'):
    parts = line.split(' | ')
    if len(parts) >= 4:
        num = int(parts[0])
        name = parts[1]
        desc = parts[2]
        gen = parts[3]
        ability_key = name.lower().replace(' ', '-').replace("'", "").replace("é", "e").replace("í", "i").replace("ó", "o").replace("ú", "u")
        abilities.append({
            'ability_key': ability_key,
            'name_en': name,
            'name_es': '',  # Placeholder
            'description_en': desc,
            'description_es': '',  # Placeholder
            'is_signature': 0,  # Placeholder
            'source_key': 'bulbapedia'
        })

# Write to CSV
with open('data_raw/abilities.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=['ability_key', 'name_en', 'name_es', 'description_en', 'description_es', 'is_signature', 'source_key'])
    writer.writeheader()
    writer.writerows(abilities)

print(f"Generated abilities.csv with {len(abilities)} abilities.")